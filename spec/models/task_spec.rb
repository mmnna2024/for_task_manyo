...
RSpec.describe Task, type: :model do
  describe 'バリデーションのテスト' do
    ...
    context 'タスクのタイトルが空文字の場合' do
      it 'バリデーションに失敗する' do
        task1.update(title: nil)
        expect(task1).to be_invalid
        expect(task1.errors.full_messages).to eq ["タイトルを入力してください"]
      end
    end

    context 'タスクの説明が空文字の場合' do
      it 'バリデーションに失敗する' do
        task1.update(content: nil)
        expect(task1).to be_invalid
        expect(task1.errors.full_messages).to eq ["内容を入力してください"]
      end
    end

    context 'タスクの終了期限が空文字の場合' do
      it 'バリデーションに失敗する' do
        task1.update(deadline_on: nil)
        expect(task1).to be_invalid
        expect(task1.errors.full_messages).to eq ["終了期限を入力してください"]
      end
    end

    context 'タスクの優先度が空文字の場合' do
      it 'バリデーションに失敗する' do
        task1.update(priority: nil)
        expect(task1).to be_invalid
        expect(task1.errors.full_messages).to eq ["優先度を入力してください"]
      end
    end

    context 'タスクのステータスが空文字の場合' do
      it 'バリデーションに失敗する' do
        task1.update(status: nil)
        expect(task1).to be_invalid
        expect(task1.errors.full_messages).to eq ["ステータスを入力してください"]
      end
    end
